//! Self-implemented AES, use to make dependencies minimize,
//! and the performance gap is negligible for the small amount of data we pass on

/// AES-128
pub struct Aes128 {
    round_keys: [[u8; 16]; 11],
}

impl Aes128 {
    /// Create an AES-128 instance with the given key
    pub fn new(key: &[u8]) -> Self {
        // Key 必须是 16 字节, 考虑到作为内部库不可能出错, 这里不做判定
        #[cfg(debug_assertions)]
        assert_eq!(key.len(), 16, "AES-128 key must be 16 bytes");
        let mut aes = Aes128 {
            round_keys: [[0; 16]; 11],
        };
        aes.key_expansion(key);
        aes
    }

    /// 密钥扩展
    fn key_expansion(&mut self, key: &[u8]) {
        // Rcon 表
        const RCON: [u8; 10] = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];
        // 复制初始密钥
        self.round_keys[0].copy_from_slice(key);

        for i in 1..11 {
            let mut temp = [0u8; 4];
            temp.copy_from_slice(&self.round_keys[i - 1][12..16]);

            // RotWord
            temp.rotate_left(1);

            // SubWord
            for t in &mut temp {
                *t = self.sub_byte(*t);
            }

            // XOR with Rcon
            temp[0] ^= RCON[i - 1];

            // 生成新的轮密钥
            for (j, &t) in temp.iter().enumerate() {
                self.round_keys[i][j] = self.round_keys[i - 1][j] ^ t;
            }

            for j in 4..16 {
                self.round_keys[i][j] = self.round_keys[i - 1][j] ^ self.round_keys[i][j - 4];
            }
        }
    }

    /// 单个字节替换
    fn sub_byte(&self, byte: u8) -> u8 {
        // S 盒
        #[rustfmt::skip]
        const S_BOX: [u8; 256] = [
            ,
        ];

        S_BOX[byte as usize]
    }

    /// 逆单个字节替换
    fn inv_sub_byte(&self, byte: u8) -> u8 {
        // 逆 S 盒
        #[rustfmt::skip]
        pub const INV_S_BOX: [u8; 256] = [
            ,
        ];

        INV_S_BOX[byte as usize]
    }

    /// 字节替换
    fn sub_bytes(&self, state: &mut [u8; 16]) {
        for s in state.iter_mut() {
            *s = self.sub_byte(*s);
        }
    }

    /// 逆字节替换
    fn inv_sub_bytes(&self, state: &mut [u8; 16]) {
        for s in state.iter_mut() {
            *s = self.inv_sub_byte(*s);
        }
    }

    /// 行移位
    fn shift_rows(&self, state: &mut [u8; 16]) {
        // 第 0 行不变
        // 第 1 行循环左移 1 位
        let temp = state[1];
        state[1] = state[5];
        state[5] = state[9];
        state[9] = state[13];
        state[13] = temp;

        // 第 2 行循环左移 2 位
        let temp1 = state[2];
        let temp2 = state[6];
        state[2] = state[10];
        state[6] = state[14];
        state[10] = temp1;
        state[14] = temp2;

        // 第 3 行循环左移 3 位 (相当于右移 1 位)
        let temp = state[15];
        state[15] = state[11];
        state[11] = state[7];
        state[7] = state[3];
        state[3] = temp;
    }

    /// 逆行移位
    fn inv_shift_rows(&self, state: &mut [u8; 16]) {
        // 第 0 行不变
        // 第 1 行循环右移 1 位
        let temp = state[13];
        state[13] = state[9];
        state[9] = state[5];
        state[5] = state[1];
        state[1] = temp;

        // 第 2 行循环右移 2 位
        let temp1 = state[2];
        let temp2 = state[6];
        state[2] = state[10];
        state[6] = state[14];
        state[10] = temp1;
        state[14] = temp2;

        // 第 3 行循环右移 3 位 (相当于左移 1 位)
        let temp = state[3];
        state[3] = state[7];
        state[7] = state[11];
        state[11] = state[15];
        state[15] = temp;
    }

    /// Galois 域乘法
    fn gf_mul(&self, a: u8, b: u8) -> u8 {
        let mut result = 0;
        let mut a = a;
        let mut b = b;

        for _ in 0..8 {
            if (b & 1) != 0 {
                result ^= a;
            }

            let hi_bit_set = (a & 0x80) != 0;
            a <<= 1;
            if hi_bit_set {
                a ^= 0x1b; // x^8 + x^4 + x^3 + x + 1
            }
            b >>= 1;
        }

        result
    }

    /// 列混合
    fn mix_columns(&self, state: &mut [u8; 16]) {
        for i in (0..16).step_by(4) {
            let a0 = state[i];
            let a1 = state[i + 1];
            let a2 = state[i + 2];
            let a3 = state[i + 3];

            state[i] = self.gf_mul(0x02, a0) ^ self.gf_mul(0x03, a1) ^ a2 ^ a3;
            state[i + 1] = a0 ^ self.gf_mul(0x02, a1) ^ self.gf_mul(0x03, a2) ^ a3;
            state[i + 2] = a0 ^ a1 ^ self.gf_mul(0x02, a2) ^ self.gf_mul(0x03, a3);
            state[i + 3] = self.gf_mul(0x03, a0) ^ a1 ^ a2 ^ self.gf_mul(0x02, a3);
        }
    }

    /// 逆列混合
    fn inv_mix_columns(&self, state: &mut [u8; 16]) {
        for i in (0..16).step_by(4) {
            let a0 = state[i];
            let a1 = state[i + 1];
            let a2 = state[i + 2];
            let a3 = state[i + 3];

            state[i] = self.gf_mul(0x0e, a0)
                ^ self.gf_mul(0x0b, a1)
                ^ self.gf_mul(0x0d, a2)
                ^ self.gf_mul(0x09, a3);
            state[i + 1] = self.gf_mul(0x09, a0)
                ^ self.gf_mul(0x0e, a1)
                ^ self.gf_mul(0x0b, a2)
                ^ self.gf_mul(0x0d, a3);
            state[i + 2] = self.gf_mul(0x0d, a0)
                ^ self.gf_mul(0x09, a1)
                ^ self.gf_mul(0x0e, a2)
                ^ self.gf_mul(0x0b, a3);
            state[i + 3] = self.gf_mul(0x0b, a0)
                ^ self.gf_mul(0x0d, a1)
                ^ self.gf_mul(0x09, a2)
                ^ self.gf_mul(0x0e, a3);
        }
    }

    /// 轮密钥加
    fn add_round_key(&self, state: &mut [u8; 16], round: usize) {
        for (i, s) in state.iter_mut().enumerate() {
            *s ^= self.round_keys[round][i];
        }
    }

    /// Encrypt a single 16-byte block
    pub fn encrypt_block(&self, block: &[u8]) -> [u8; 16] {
        let mut state = [0u8; 16];
        state.copy_from_slice(block);

        // 初始轮密钥加
        self.add_round_key(&mut state, 0);

        // 9轮主循环
        for round in 1..10 {
            self.sub_bytes(&mut state);
            self.shift_rows(&mut state);
            self.mix_columns(&mut state);
            self.add_round_key(&mut state, round);
        }

        // 最终轮
        self.sub_bytes(&mut state);
        self.shift_rows(&mut state);
        self.add_round_key(&mut state, 10);

        state
    }

    /// Decrypt a single 16-byte block
    pub fn decrypt_block(&self, block: &[u8]) -> [u8; 16] {
        let mut state = [0u8; 16];
        state.copy_from_slice(block);

        // 初始轮密钥加
        self.add_round_key(&mut state, 10);

        // 9轮主循环
        for round in (1..10).rev() {
            self.inv_shift_rows(&mut state);
            self.inv_sub_bytes(&mut state);
            self.add_round_key(&mut state, round);
            self.inv_mix_columns(&mut state);
        }

        // 最终轮
        self.inv_shift_rows(&mut state);
        self.inv_sub_bytes(&mut state);
        self.add_round_key(&mut state, 0);

        state
    }

    /// AES encrypt, use ECB mode, PKCS7Padding
    pub fn encrypt_ecb(&self, data: &[u8]) -> Vec<u8> {
        let mut input = data.to_vec();

        // PKCS7Padding
        let padding_len = 16 - (input.len() % 16);
        input.extend(vec![padding_len as u8; padding_len]);

        // 加密每个块
        let mut output = Vec::with_capacity(input.len());
        for chunk in input.chunks(16) {
            let encrypted_block = self.encrypt_block(chunk);
            output.extend_from_slice(&encrypted_block);
        }

        output
    }

    /// AES decrypt, use ECB mode, PKCS7Padding
    pub fn decrypt_ecb(&self, data: &[u8]) -> Vec<u8> {
        // 解密块
        let mut output = Vec::with_capacity(data.len());
        for chunk in data.chunks(16) {
            let decrypted_block = self.decrypt_block(chunk);
            output.extend_from_slice(&decrypted_block);
        }

        // 移除 PKCS7 填充
        let padding_len = *output.last().expect("PKCS7 padding should not panic") as usize;
        if padding_len <= 16 {
            output.truncate(output.len() - padding_len);
        }

        output
    }

    /// AES encrypt, use CBC mode, ZeroPadding
    pub fn encrypt_cbc(&self, data: &[u8], iv: &[u8]) -> Vec<u8> {
        let mut input = data.to_vec();

        // ZeroPadding
        let padding_len = 16 - (input.len() % 16);
        input.extend(vec![0u8; padding_len]);

        let mut output = Vec::with_capacity(input.len());
        let mut prev_block = iv.to_vec();

        for chunk in input.chunks(16) {
            // XOR 上一个密文块
            let mut block = [0u8; 16];
            for i in 0..16 {
                block[i] = chunk[i] ^ prev_block[i];
            }

            // 加密块
            let encrypted_block = self.encrypt_block(&block);
            output.extend_from_slice(&encrypted_block);
            prev_block = encrypted_block.to_vec();
        }

        output
    }
}
